...
call gn gen out/Release --args="angle_build_all=false is_debug=false angle_has_frame_capture=false angle_enable_gl=false angle_enable_d3d11=false angle_enable_d3d9=false angle_enable_null=false angle_enable_vulkan=true" || exit /b 1
...
copy /y angle.src\out\Release\libEGL.dll.pdb       angle\bin        1>nul 2>nul
copy /y angle.src\out\Release\libGLESv1_CM.dll.pdb angle\bin        1>nul 2>nul
copy /y angle.src\out\Release\libGLESv2.dll.pdb    angle\bin        1>nul 2>nul
...
if "%GITHUB_WORKFLOW%" neq "" (
  ...
  %SZIP% a -mx=9 angle-src-%BUILD_DATE%.zip angle.src/src || exit /b 1
  %SZIP% a -mx=9 angle-%BUILD_DATE%.zip angle || exit /b 1

  rem echo "ANGLE_COMMIT=%ANGLE_COMMIT%" >> $GITHUB_OUTPUT
  rem echo "BUILD_DATE=%BUILD_DATE%" >> $GITHUB_OUTPUT
